...
export function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      // errorMessage() surfaces the backend's actual code (wrong credentials, rate-limited,
      // password over bcrypt's length limit, ...) when there is one, instead of collapsing
      // everything but a 401 into one generic message.
      setError(errorMessage(err, t("auth.login.errorGeneric")));
    } finally {
      setSubmitting(false);
    }
  }
