...
# healthy 30s timeout into several minutes. Binding the local address to the IPv4 wildcard makes
...
# call — see llm_service.py's identical comment: closing a Client that was given an explicit
# transport= force-closes that transport's connection pool, including connections a *different*,
# concurrent call on the same shared transport is still using. A single long-lived Client avoids
# that and gives every call real keep-alive.
