...
def list_keys(current_user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    stt_usage = dict(
        session.exec(
            select(Project.stt_api_key_id, func.count(Project.id))
            .where(Project.user_id == current_user.id, Project.stt_api_key_id.is_not(None))
            .group_by(Project.stt_api_key_id)
        ).all()
    )
    usage = {
        key_id: llm_usage.get(key_id, 0) + tts_usage.get(key_id, 0) + stt_usage.get(key_id, 0)
        for key_id in set(llm_usage) | set(tts_usage) | set(stt_usage)
    }
...
def delete_key(
    ...
    referencing_stt = session.exec(select(Project).where(Project.stt_api_key_id == key.id)).all()
    for project in referencing_stt:
